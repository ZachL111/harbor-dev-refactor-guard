...
`harbor-dev-refactor-guard` has a compact core implementation, golden fixtures, extended examples, and a repository audit script. The model is intentionally small enough to inspect by hand while still exercising real control flow and validation.
...
The extended example set currently includes 0 accept cases and 6 review cases. This is meant to exercise both normal and cautious paths without depending on external systems.
