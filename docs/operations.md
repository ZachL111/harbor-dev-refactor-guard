...
`harbor-dev-refactor-guard` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies refactor behavior through seeded input scenarios, with deterministic summary checks and bounded memory input sets.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 3.
