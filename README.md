...
harbor-dev-refactor-guard is a Rust project for developer tools. It focuses on this technical goal: Build a Rust toolkit that studies refactor behavior through seeded input scenarios, with deterministic summary checks and bounded memory input sets.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 178, risk penalty 6, latency penalty 4, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
